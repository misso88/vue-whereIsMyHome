  ...
  <div class="py-8">
    ...
    <v-row justify="space-between" class="my-4">
      <v-col>
        ...
        <div class="grey--text">{{ article.nickname }} | {{ article.registerTime }}</div>
      </v-col>
      <v-row justify="end" align="center" class="mr-3">
        <v-btn :to="{ name: 'boardupdate', params: { articleNo: articleNo } }" class="ma-1">수정</v-btn>
        <v-btn @click="deleteArticle" class="ma-1">삭제</v-btn>
      </v-row>
    </v-row>
    <v-divider />
    <v-container class="my-4" v-html="article.content"> </v-container>
    <v-divider />
    <v-row justify="center" class="my-4">
      <v-btn @click="moveList" class="ma-2" large color="primary" width="150px">목록</v-btn>
    </v-row>
    <v-row align="center" class="grey lighten-4 pa-6 my-6">
      <v-text-field placeholder="댓글을 입력해주세요." outlined hide-details class="mr-4 white"></v-text-field>
...
export default {
  ...
  methods: {
    ...
    moveList() {
      ...
      let name = category == 2 ? "notice" : category == 3 ? "helpcenter" : "community";
      this.$router.push({
        name: name,
      });
    },
