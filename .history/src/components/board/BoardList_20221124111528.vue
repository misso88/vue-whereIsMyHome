...
export default {
  ...
  created() {
    ...
    let param = {
      category: category == 0 ? "share" : category == 1 ? "local" : category == 2 ? "notice" : "one_on_one",
    ...
    countListSize(
      ...
      ({ data }) => {
        ...
        let pageLength =
          data / this.$store.state.boardStore.pageSize + (data % this.$store.state.boardStore.pageSize == 0 ? 0 : 1);
        this.SET_PAGE_LENGTH(pageLength);
      },
      (error) => {
        console.log(error);
      }
