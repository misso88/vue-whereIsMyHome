<template>
  <v-card height="100vh" class="pa-6">
    ...
    <v-row class="pa-4" style="position: relative">
      <v-avatar size="100%" class="pa-6">
        <img src="https://cdn.vuetifyjs.com/images/john.jpg" alt="John" />
      </v-avatar>
      <v-row class="mb-8 mr-8" justify="end" align="end">
        <v-btn absolute fab x-large><v-icon x-large>mdi-pencil</v-icon></v-btn>
      </v-row>
    </v-row>
    <v-divider class="my-4"></v-divider>
    <v-tabs v-model="tab" color="grey" vertical hide-slider center-active>
    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
      ...
      <v-card>
        ...
        <v-card-actions>
          <v-spacer></v-spacer>
          <v-btn color="blue darken-1" text @click="clickUnRegister"> 확인 </v-btn>
...
export default {
  ...
  data() {
    return {
      ...
      dialog: false,
      member: {
        emailId: this.$store.state.memberStore.user.emailId,
      },
    };
  },
  methods: {
