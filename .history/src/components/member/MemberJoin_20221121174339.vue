<template>
  <v-card class="pa-8" light width="800" max-height="800"><v-card-title class="justify-center text-h4 mb-4">로그인</v-card-title>
    
    <v-row>
      <v-spacer />
      <v-col :cols="mainWidth">
        <h2 class="my-10 text-center">회원가입</h2>
        <v-form>
          <label>이메일</label>
          <v-row class="ma-0" align="center">
            <v-text-field v-model="emailId" outlined></v-text-field>
            <div class="mb-7 mx-1">@</div>
            <v-select v-model="emailDomain" :items="emailDomains" outlined>
            </v-select>
          </v-row>
          <label>비밀번호</label>
