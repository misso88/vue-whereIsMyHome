...
let boardStore = {
  ...
  actions: {
    ...
    async searchArticles(context) { 
      await axios.get(`${process.env.VUE_APP_BACKEND_SERVER_URL}/board/${state.categoryTab}/${this.state.pgno}/${this.state.pageSize}/${this.state.option}/${this.state.keyword}`)
        .then(({ data }) => { 
          context.commit("SET_BOARD_LIST", data);
        });
    },
